...
const VerTicket = () => {
    ...
    useEffect(()=>{
        const getCotizacion = async()=>{
            const {data} = await axios.get(backend()+'/api/ticket/'+id)
            .catch((err)=>{
                console.log(err)
                sweetalert2.fire({
                    icon: 'error',
    ...
    const eliminar = async()=>{
        sweetalert2.fire({
            title: '¿Estas seguro que quieres borrar este ticket?',
            ...
            if (result.isConfirmed) {
              ...
              await axios.delete(backend()+'/api/ticket/'+id)
    ...
    const descargar = async()=>{
        ...
        //GENERAR
        const {data} = await axios.post(backend()+'/descargar/ticket/'+id);
        if(data.exito){
                let {data} = await axios.get(backend()+'/descargar/ticket/'+id, {
                    responseType: 'blob'
                });
                ...
                await axios.delete(backend()+'/descargar/ticket/'+id);
        }else{
            Toast.fire({
                icon: 'error',
    ...
    const enviar = async()=>{
        const {data} = await axios.post(backend()+'/api/enviar/ticket/'+id);
        if(data.exito){
            ...
            }).then((result)=>window.location.href = '/ticket/'+id+'/false');
        }else{
            sweetalert2.fire({
                icon: 'error',
                iconColor: 'red',
                title: 'ERROR: '+data.message,
                text: 'Ha ocurrido un error al enviar el ticket',
                color: 'black',
                footer: '<p>Si el problema persiste reporte el error al correo: <a href="mailto:cotiapp.dev@gmail.com">cotiapp.dev@gmail.com</a></p>',
                confirmButtonText: 'Aceptar',
                confirmButtonColor: '#F5305C'
    ...
    if(items.length === 0)
        ...
        return (
            <div className='div-ver-cotizacion-main'>
                {linkVolver()}
                <Titulo>Ticket #{id}<br/><small>Folio #{cotizacion.folio}</small></Titulo>
                ...
                <table>
                    <tr>
                        ...
                        <th style={{backgroundColor: cotizacion.color+'80'}} className='borde-abajo-none'>Forma de Pago</th>

                    </tr>

                    <tr>
                        <th style={{width: '16%'}} className='borde-abajo-none'>{cliente.nombre}</th>
                        <th style={{width: '16%'}} className='borde-abajo-none'>{cliente.empresa}</th>
                        <th style={{width: '36%'}} className='borde-abajo-none'>{cliente.correo}</th>
                        <th style={{width: '16%'}} className='borde-abajo-none'>{cliente.telefono}</th>
                        <th style={{width: '16%'}} className='borde-abajo-none'>{cotizacion.pago}</th>
